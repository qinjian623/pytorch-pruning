...
class FilterPrunner:
    ...
    def compute_rank(self, grad):
        ...
        values = \
            torch.sum((activation * grad),
                      dim=0,
                      keepdim=True).sum(dim=2,
                                        keepdim=True).sum(dim=3,
                                                          keepdim=True)
        # print(values.shape)
        values = values[0, :, 0, 0].data
...
if __name__ == '__main__':
    ...
    fine_tuner = PrunningFineTunerVGG16(args.train_path, args.test_path, model)
